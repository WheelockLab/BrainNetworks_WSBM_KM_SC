function[HD_rep] = HD_inter(kmeans_results, wsbm_results, sc_results)
% This function calculates the Hamming distance values between all pairs of replications
...
parfor k = 2:K_valid
   ...
   HD_k_s{k} = hamming_distance(sc_results.idx(:,:,k),kmeans_results.idx(:,:,k))
   HD_w_k{k} = hamming_distance(valid_id{k},kmeans_results.idx(:,:,k));
   HD_w_s{k} = hamming_distance(valid_id{k},sc_results.idx(:,:,k));
end


HD_rep.kmeans_sc = HD_k_s;
HD_rep.wsbm_kmeans = HD_w_k;
HD_rep.wsbm_sc = HD_w_s;
