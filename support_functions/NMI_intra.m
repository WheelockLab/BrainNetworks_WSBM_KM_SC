function[NMI_rep] = NMI_intra(kmeans_results, wsbm_results, sc_results)
...
parfor k = 2:K_valid
   [~, NMI_s{k}] = partition_distance(sc_results.idx(:,:,k),sc_results.idx(:,:,k));
   [~, NMI_k{k}] = partition_distance(kmeans_results.idx(:,:,k),kmeans_results.idx(:,:,k));
   [~, NMI_w{k}] = partition_distance(valid_id{k},valid_id{k});
end

NMI_rep.kmeans = NMI_k;
NMI_rep.wsbm = NMI_w;
NMI_rep.sc = NMI_s;
